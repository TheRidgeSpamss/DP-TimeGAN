...
def train_test_divide (data_x: np.ndarray, data_x_hat: np.ndarray, data_t: np.ndarray, data_t_hat: np.ndarray, train_rate: float = 0.8):
	"""
...
def save_results_to_excel(filename, metric_results, oneclass_results, parameters):
    """
    Save the metric results to an Excel file.

    Args:
        filename (str): The file path for saving the Excel file.
        metric_results (dict): The dictionary containing the learned metric results.
        oneclass_results (dict): The dictionary containing the statistical metric results.
    """
    # Define the excel file path
    excel_file = "results.xlsx"
    script_dir = os.path.dirname(__file__)
    results_dir = os.path.join(script_dir, 'Excel_Results')
    excel_path = os.path.join(results_dir, excel_file)
    
    # Ensure the directory exists
    os.makedirs(results_dir, exist_ok=True)
    
    sine_no = parameters["sine_no"] if parameters["data_name"] == 'sines' else None
    # Create a dataframe from the metric results
    df = pd.DataFrame({
        #"Filename": [filename],
        "Dataset": [parameters["data_name"]],
        "#Epochs": [parameters["iterations"]],
        "Noise SD": [parameters["noise_sd"]] if "noise_sd" in parameters else None,
        "Metric Trials": [parameters["metric_iteration"]],
        "Discrim. Score": [metric_results['discriminative']],
        "Pred. Score": [metric_results['predictive']],
        "Accuracy": [metric_results["downstream_accuracy"]] if parameters["data_name"] in ["ckd", "eicu"] else None,
        "α-Precision": [oneclass_results["alpha_precision"]],
        "β-Recall": [oneclass_results["beta_recall"]],
        "Authenticity": [oneclass_results["authenticity"]],
        "Use DP": [parameters["use_dp"]],
        "Epsilon": [parameters["eps"]] if parameters["use_dp"] else None,
        "Random Seed": [parameters["seed"]] if "seed" in parameters else None,
        "Model Name": [parameters["model_name"]] if "model_name" in parameters else None
    })
